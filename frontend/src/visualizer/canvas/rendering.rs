...
impl WaveformRenderer {
    ...
    pub fn render_frame(&mut self) {
        ...
        self.add_waveforms(&mut objects, width, height);
    ...
    fn add_waveforms(&self, objects: &mut Vec<Object2d>, width: u32, height: u32) {
        ...
        // ✅ CORRECT from f8f1cf4: Use get_full_file_range() for proper timeline bounds
        ...
        for (index, variable) in variables.iter().enumerate() {
            ...
            self.add_signal_value_blocks(objects, variable, y_position, row_height, width as f32, timeline_min, timeline_max, &theme_colors);
        ...
        self.add_timeline_row(objects, width as f32, height as f32, row_height, total_rows, timeline_min, timeline_max, &theme_colors);
    }
    ...
    fn add_signal_value_blocks(
        ...
        let time_value_pairs = self.get_signal_transitions_for_variable(variable, timeline_min, timeline_max);
                ...
                if rect_start_x >= -10.0 && rect_start_x <= canvas_width as f64 + 10.0 {
                    ...
                    objects.push(
                        Rectangle::new()
                            ...
                            .size(1.0, row_height)
                            .color(theme_colors.cursor_color.0, theme_colors.cursor_color.1, theme_colors.cursor_color.2, theme_colors.cursor_color.3)
                            .into()
                    );
                }
                continue;
            ...
            if text_width >= 10.0 && text_height >= 8.0 {
                objects.push(
                    Text::new()
                        ...
                        .position((rect_start_x + text_padding as f64) as f32, (y_position + row_height / 3.0) as f32)
    ...
    fn get_signal_transitions_for_variable(
        ...
        let transitions = match variable_name {
            ...
            "C" => vec![
                (timeline_min as f32, SignalValue::Present("1010".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.3, SignalValue::Present("0110".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.7, SignalValue::Present("1100".to_string())),
            ],
            "S" => vec![
                (timeline_min as f32, SignalValue::Present("11".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.4, SignalValue::Present("00".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.8, SignalValue::Present("10".to_string())),
            ],
            "Os" => vec![
                (timeline_min as f32, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.2, SignalValue::Present("1".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.6, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.9, SignalValue::Present("1".to_string())),
            ],
            ...
            "B" => vec![
                (timeline_min as f32, SignalValue::Present("11".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.5, SignalValue::Present("00".to_string())),
            ],
            "clk" => vec![
                (timeline_min as f32, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.1, SignalValue::Present("1".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.2, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.3, SignalValue::Present("1".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.4, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.5, SignalValue::Present("1".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.6, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.7, SignalValue::Present("1".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.8, SignalValue::Present("0".to_string())),
                (timeline_min as f32 + (timeline_max - timeline_min) as f32 * 0.9, SignalValue::Present("1".to_string())),
            ],
            _ => vec![
                (timeline_min as f32, SignalValue::Present("0".to_string())),
    ...
    fn add_timeline_row(
        ...
        objects.push(
            Rectangle::new()
                ...
                .color(theme_colors.neutral_2.0, theme_colors.neutral_2.1, theme_colors.neutral_2.2, 1.0)
                .into()
        );
        ...
        for tick_index in 0..actual_tick_count {
            ...
            objects.push(
                Rectangle::new()
                    ...
                    .size(1.0, 8.0)
                    .color(theme_colors.neutral_12.0, theme_colors.neutral_12.1, theme_colors.neutral_12.2, theme_colors.neutral_12.3)
                    .into()
            );
    ...
    fn get_time_unit_for_range(&self, min_time: f64, max_time: f64) -> TimeUnit {
        ...
        if range < 1e-6 {  // Less than 1 microsecond
            TimeUnit::Nanosecond
        } else if range < 1e-3 {  // Less than 1 millisecond
            TimeUnit::Microsecond
        } else if range < 1.0 {   // Less than 1 second
