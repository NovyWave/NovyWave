...
pub fn waveform_canvas(
    ...
    setup_canvas_event_connections(
        waveform_canvas,
        selected_variables,
        waveform_timeline,
        app_config,
...
fn create_canvas_element(
    waveform_canvas: &WaveformCanvas,
    _waveform_timeline: &crate::visualizer::timeline::timeline_actor::WaveformTimeline,
) -> impl Element {
        ...
        async move |state_handle| {
            if !state_handle.get() {
                let mut dimensions_stream =
                    waveform_canvas.canvas_dimensions_changed_relay.subscribe();
