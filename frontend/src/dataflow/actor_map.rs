...
use std::sync::Arc;
use zoon::{
    MutableBTreeMap, Signal, SignalExt, SignalMap, SignalMapExt, SignalVec, SignalVecExt, Task,
    TaskHandle,
};
...
mod tests {
    ...
    async fn test_actor_map_basic_operations() {
        ...
        // Test value signals
        let key1_value = cache
            .value_signal("key1".to_string())
            .to_stream()
            .next()
            .await
            .unwrap();
        let key2_value = cache
            .value_signal("key2".to_string())
            .to_stream()
            .next()
            .await
            .unwrap();
        ...
        let key1_removed = cache
            .contains_key_signal("key1".to_string())
            .to_stream()
            .next()
            .await
            .unwrap();
    ...
    async fn test_actor_map_value_signal() {
        ...
        let initial = cache
            .value_signal("test_key".to_string())
            .to_stream()
            .next()
            .await
            .unwrap();
    ...
    async fn test_actor_map_handle_operations() {
        ...
        let incremented = cache
            .value_signal("counter".to_string())
            .to_stream()
            .next()
            .await
            .unwrap();
