use crate::error_display::{ErrorAlert, log_error_console_only};
    ...
    match down_msg {
        ...
        DownMsg::ParsingProgress { file_id, progress } => {
            tracked_files
                ...
                .send((file_id, progress, LoadingStatus::Parsing));
        }
        ...
        DownMsg::ParsingError { file_id, error } => {
            ...
            let filename = { "Unknown file".to_string() };
        ...
        DownMsg::DirectoryContents { path, items } => {
            zoon::println!(
                "📥 CONNECTION: Received DirectoryContents for {} with {} items",
                path,
                items.len()
            );
        ...
        DownMsg::SignalValues { file_path, results } => {
            zoon::println!(
                "📨 Received SignalValues for {} with {} results",
                file_path,
                results.len()
            );
        }
